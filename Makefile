...
# Detect libretro invocation via -f Makefile.libretro or LIBRETRO=1
ifneq (,$(findstring Makefile.libretro,$(MAKEFILE_LIST)))
LIBRETRO ?= 1
endif
...
else ifeq ($(LIBRETRO), 1)
# For libretro: default platform is native PC (platform empty) unless platform=miyoo
ifeq ($(platform),)
LINUX ?= YES
endif
else
# For standalone: default is miyoo unless LINUX=YES
ifneq ($(LINUX), YES)
CHAINPREFIX ?= /opt/miyoo
CROSS_COMPILE ?= $(CHAINPREFIX)/usr/bin/arm-linux-
endif
...
ifeq ($(LIBRETRO), 1)
  ...
  # Pick correct CORE by platform (disable default core below to use UAE)
  ifeq ($(platform), miyoo)
    LDFLAGS     = -lz -lpthread
    OPTIMIZE_CFLAGS = -g -O3 -fno-PIC
    MORE_CFLAGS += -DHAVE_KBDJOYPAD -DUSE_CAST_UNSIGNED
    ifndef UAE_CORE
      MORE_CFLAGS += -DFAST_COMPILER
      ifndef FAME_CORE
        CYCLONE_CORE = 1
      endif
    endif
  ...
  MORE_CFLAGS += -D__LIBRETRO__ -DNO_VKBD -DUSE_ALL_LINES
  ...
  #MORE_CFLAGS += -DUSE_ALL_LINES ## in standalone it disables entirely status-bar (instead of making it transparent)
...
clean:
	$(RM) $(PROG) $(OBJS) uae4all $(NAME)_libretro.so $(NAME)_libretro.a
	rm -rf $(RELEASEDIR)
	rm -f *.ipk
	rm -f *.zip

.PHONY: all clean almostclean run package zip ipk gm2xpkg-ipk opk install libretro
