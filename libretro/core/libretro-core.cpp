...
//      { "uae4all_floppy_speed",   "Floppy speed; 100|200|400|800", },
      { "uae4all_throttle",   "Throttle / Optimize level (need Restart); none|20|40|60|80|100", },
...
void update_prefs_retrocfg(void)
{
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      ...
      if (strcmp(var.value, "4") == 0)
      {
         mainMenu_frameskip = 4;
      }
      if (strcmp(var.value, "5") == 0)
      {
